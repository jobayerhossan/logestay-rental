...
<div>
   <button class="<?php echo $cityitem_class; ?> city_item" data-id="<?php echo get_the_ID(); ?>">
      ...
      <div class="absolute bottom-0 left-0 right-0 p-6 md:p-8 text-white">
         ...
         <?php else : ?>
            <div class="inline-flex items-center gap-2 bg-gray-600/80 backdrop-blur-md px-4 py-2 rounded-full"><span class="text-sm md:text-base font-semibold">Aucun logement disponible</span></div>
         <?php endif; ?>
      </div>
      <div class="absolute top-4 right-4 bg-amber-500 text-white px-4 py-2 rounded-full text-sm font-bold shadow-lg <?php echo $select_class; ?> selected_item"><?php _e('Selected', 'logestay'); ?></div>
   </button>
</div>